...
function Questionario() {
  ...
  useEffect(() => {
    if (!hasInitialized.current) {
      ...
      addBotMessage("Olá! Sou a Camila, atendente da Prefeitura de Barueri. Estou aqui para ajudar a verificar a sua elegibilidade para o Auxílio Pet.\n\nVamos começar a análise do seu perfil.");
      setTimeout(() => processStep("Q1"), 1000);
    }
  }, []);

 // --- NOVA FUNÇÃO DE VOZ DA CAMILA (TENTANDO VOZES PREMIUM/NEURAL) ---
  const speak = (text: string) => {
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      const utterance = new SpeechSynthesisUtterance(text);
      utterance.lang = 'pt-BR'; 
      utterance.rate = 0.9; // <-- 0.9 deixa um pouquinho mais lento e natural do que 1.0

      const voices = window.speechSynthesis.getVoices();
      
      // 1º Tenta achar vozes de Alta Qualidade (Google Premium ou Microsoft Neural - Francisca)
      const premiumVoice = voices.find(v => 
        v.lang.includes('pt-BR') && 
        (v.name.includes('Google') || v.name.includes('Neural') || v.name.includes('Francisca'))
      );

      // 2º Se não achar premium, tenta as normais
      const normalFemaleVoice = voices.find(v => 
        v.lang.includes('pt-BR') && 
        (v.name.toLowerCase().includes('maria') || v.name.toLowerCase().includes('luciana'))
      );

      if (premiumVoice) {
        utterance.voice = premiumVoice;
      } else if (normalFemaleVoice) {
        utterance.voice = normalFemaleVoice;
      } else {
        utterance.pitch = 1.1; // Ajuste leve se só tiver a voz padrão de robô
      }
  ...
  const addBotMessage = (text: string) => {
    ...
    speak(text); // Camila fala o texto automaticamente
  };
  ...
  const saveAnswers = async (finalData: any) => {
    try {
      ...
      if (registrationId) {
        ...
        if (questError) console.error("Erro ao salvar questionário:", questError);
        ...
        if (regError) console.error("Erro ao salvar dados bancários:", regError);
      }
        ...
        <div className="space-y-6">
          {messages.map((message) => (
            <div key={message.id} className={`flex gap-3 ${message.type === 'user' ? 'justify-end' : 'justify-start'} animate-fadeInUp`}>
              {message.type === 'bot' && (
                <img src="/atendente.jpg" alt="Camila" className="w-10 h-10 rounded-full object-cover flex-shrink-0 mt-1 shadow-sm" />
              )}
          ...
          {isTyping && (
            <div className="flex gap-3 animate-fadeInUp">
              <img src="/atendente.jpg" alt="Camila" className="w-10 h-10 rounded-full object-cover flex-shrink-0 shadow-sm" />
